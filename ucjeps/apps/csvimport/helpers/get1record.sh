...
curl -4 -S --stderr curl2.tmp -X GET "$CSPACEURL/$SERVICEITEMS?pgSz=1" --basic -u "$CSPACEUSER" -H "$CONTENT_TYPE" > curl.tmp
perl -pe 's/<list/\n<list/g' curl.tmp | perl -ne 'while (s/<list\-item>.*?<csid>(.*?)<.*?<$ENV{EXTRACT}.*?>(.*?)<.*?<\/list\-item>//) { print "$1\n" }' > csid.tmp
CSID=`cat csid.tmp`

curl -4 -S --stderr curl.junk -X GET "$CSPACEURL/$SERVICEITEMS/$CSID" --basic -u "$CSPACEUSER" -H "$CONTENT_TYPE" > $1.tmp
