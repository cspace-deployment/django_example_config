...
if __name__ == "__main__":
    ...
    print getobjinfo('1-504')

    print '\nkeyinfo\n'
    # Kroeber, 20A, X  1,  1
    # Kroeber, 20AMez, 128 A
    for i, loc in enumerate(getlocations('Kroeber, 20A, X  1,  3', '', 1, 'keyinfo','pahma')):
    ...
    print '\nrefnames\n'
    print getrefname('concepts_common', 'zzz')
    print getrefname('concepts_common', '')
    print getrefname('concepts_common', 'Yurok')
    print findrefnames('places_common', ['zzz', 'Sudan, Northern Africa, Africa'])
    print '\ncurrentlocation\n'
    print findcurrentlocation('c65b2ffa-6e5f-4a6d-afa4-e0b57fc16106')

    print '\nset of locations\n'
    for loc in getloclist('set', 'Kroeber, 20A, W B', '', 10):
        print loc

    print '\nlocations by prefix\n'
    for loc in getloclist('prefix', 'Kroeber, 20A, W B', '', 1000):
        print loc

    print '\nlocations by range\n'
    for loc in getloclist('range', 'Kroeber, 20A, W B2, 1', 'Kroeber, 20A, W B5, 11', 1000):
        print loc

    print '\nobjects\n'
    for i, loc in enumerate(getlocations('Regatta, A150, South Nexel Unit 6, C', '', 1, 'inventory','pahma')):
        print 'location', i + 1, loc[0:6]

    print '\nkeyinfo\n'
    for i, loc in enumerate(getlocations('Kroeber, 20AMez, 128 A', '', 1, 'keyinfo','pahma')):
        print 'location', i + 1, loc[0:12]
